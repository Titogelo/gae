...
//        if (qu.getCurrentAccount().getAccountType().equals(Account.ECO)) {
            (new SendToLearningLocker(authToken, action.getRunId(), action.getAction(), action.getUserEmail(), action.getTimestamp(), run.getGame().getTitle(), action.getGeneralItemId())).scheduleTask();
//        }
